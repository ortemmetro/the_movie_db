...
enum ApiClientExceptionType { network, auth, other, expiredSession }
...
class ApiClient {
  ...
  Future<T> _get<T>(
    ...
    try {
      ...
      final dynamic json = (await response.myJsonDecode());
  ...
  void _validateResponse(HttpClientResponse response, dynamic json) {
    if (response.statusCode == 401) {
      ...
      if (code == 30) {
        ...
        throw ApiClientException(ApiClientExceptionType.expiredSession);
      } else {
        throw ApiClientException(ApiClientExceptionType.other);
      }
    }
...
extension HttpClientResponseJsonDecode on HttpClientResponse {
  Future<dynamic> myJsonDecode() async {
    return transform(utf8.decoder)
        .toList()
        .then((value) => value.join())
        .then<dynamic>((v) => jsonDecode(v));
  }
}
